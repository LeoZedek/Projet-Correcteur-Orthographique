...
int sontEgauxEntiers(CO_TableauBooleens entierGeneres, CO_TableauBooleens entierVerifies){
  ...
  if (CO_obtenirLongeurTabEntiers(entierGeneres) != CO_obtenirLongeurTabEntiers(entierVerifies)) {
    egaux = FALSE;
  }
  else {
    for (i=0; i<CO_obtenirLongeurTabEntiers(entierGeneres); i++){
...
int sontEgauxMots(MOT_TableauDeMots motsGeneres, MOT_TableauDeMots motsVerifies){
  int i;
  int egaux = TRUE;
  if (MOT_obtenirLongeurTabMots(motsGeneres) != MOT_obtenirLongeurTabMots(motsVerifies)) {
    egaux = FALSE;
  }
  else {
    for (i=0; i < MOT_obtenirLongeurTabMots(motsGeneres); i++){
...
void test_sontPresents(void)
{
  ...
  m = creerMot("correction");
  DICTIONNAIRE_ajouterMot(&dico, m);

  m = creerMot("dromadaire");
  DICTIONNAIRE_ajouterMot(&dico, m);
...
void test_phraseEnMots(void)
{
	...
	CO_MotsDansPhrase motsEtPositions = CO_phraseEnMots(phrase);

	MOT_TableauDeMots motsExacts = MOT_tableauDeMotsVide();
	CO_TableauPositions positionsExactes = CO_tableauDEntiersVide();

	MOT_ajouterMot(&motsExacts, creerMot("bonjour"));
	MOT_ajouterMot(&motsExacts, creerMot("je"));
	MOT_ajouterMot(&motsExacts, creerMot("suis"));
	MOT_ajouterMot(&motsExacts, creerMot("une"));
	MOT_ajouterMot(&motsExacts, creerMot("pizza"));
